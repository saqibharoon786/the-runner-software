...
import {
  ...
  customSoftwareFaqs,
  finalCta,
} from "@/data/custom-software-development-page";
import { NAVY } from "@/lib/brand";

function PageSchema() {
  const baseUrl = "https://[yourdomain].com";
  const pageUrl = `${baseUrl}${customSoftwareSeo.canonicalPath}`;
  const schema = {
    "@context": "https://schema.org",
    "@graph": [
      {
        "@type": "WebPage",
        name: customSoftwareSeo.h1,
      ...
      },
      {
        ...
        itemListElement: [
          ...
          { "@type": "ListItem", position: 4, name: "Custom Software Development", item: pageUrl },
        ],
      },
      {
        "@type": "FAQPage",
        mainEntity: customSoftwareFaqs.map((f) => ({
          "@type": "Question",
          name: f.q,
...
function FaqAnswer({ index, answer }: { index: number; answer: string }) {
  if (index === 25) {
    return (
      <p>
        Custom software refers to any software built specifically for one organization&apos;s needs; enterprise software development typically refers to larger-scale, organization-wide systems with more complex integration and compliance requirements. Learn more on our{" "}
        <Link href={getServicePath("enterprise-software-development")} className="font-semibold text-[#0f1a4e] hover:underline">enterprise software development</Link> page.
      </p>
    );
  }
  if (index === 26) {
    return (
      <p>
        Yes. Our{" "}
        <Link href={getServicePath("software-consulting")} className="font-semibold text-[#0f1a4e] hover:underline">software consulting</Link> service is designed specifically to help businesses evaluate options and define the right technical approach before committing to a build.
      </p>
    );
  }
  if (index === 27) {
    return (
      <p>
        Yes — that engagement is typically structured under our{" "}
        <Link href={getServicePath("software-product-development")} className="font-semibold text-[#0f1a4e] hover:underline">software product development</Link> service, which is built around product-market considerations in addition to engineering.
      </p>
    );
  }
  return <p>{answer}</p>;
...
export default function CustomSoftwareDevelopmentPage() {
  const [openFaq, setOpenFaq] = useState<string | null>(customSoftwareFaqs[0]?.q ?? null);
  const portfolioPreview = filterProjects("custom-software").slice(0, 3);
      ...
      <main>
        ...
        <section className="relative overflow-hidden border-b border-slate-100" aria-labelledby="page-heading">
          <div className="mx-auto max-w-7xl px-6 py-12 md:py-16">
            <nav aria-label="Breadcrumb" className="flex flex-wrap items-center gap-1 text-xs font-medium text-slate-500">
              ...
              <ChevronRight className="h-3 w-3" aria-hidden="true" />
              <span className="text-[#0f1a4e]">Custom Software Development</span>
            </nav>
            ...
            <div className="mt-10 grid grid-cols-1 gap-10 lg:grid-cols-2 lg:items-center">
              <MotionSection>
                ...
                <h1 id="page-heading" className="mt-6 text-4xl font-extrabold leading-[1.1] tracking-tight text-[#0b1437] md:text-5xl">
                  {customSoftwareSeo.h1}
                </h1>
                <p className="mt-4 text-lg font-semibold text-[#0f1a4e]">{customSoftwareHero.subheading}</p>
                <p className="mt-4 text-[15px] leading-relaxed text-slate-600">{customSoftwareHero.valueProposition}</p>
                <div className="mt-8 flex flex-wrap gap-4">
                  <Link href="/contact" className="inline-flex items-center gap-2 rounded-lg px-6 py-3.5 text-sm font-semibold text-white shadow-xl transition hover:-translate-y-0.5" style={{ backgroundColor: NAVY }}>
                    {customSoftwareHero.primaryCta} <ArrowRight className="h-4 w-4" aria-hidden="true" />
                  </Link>
                  <a href="#process" className="inline-flex items-center gap-2 rounded-lg border border-slate-200 bg-white px-6 py-3.5 text-sm font-semibold text-[#0b1437] transition hover:bg-slate-50">
                    {customSoftwareHero.secondaryCta}
                  </a>
                </div>
                <p className="mt-8 text-sm leading-relaxed text-slate-600">{customSoftwareHero.trustStatement}</p>
              </MotionSection>
              <motion.div initial={{ opacity: 0, scale: 0.98 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5 }}>
                <img
                  src="https://images.unsplash.com/photo-1522071820081-009f0129c71c?w=1200&auto=format&fit=crop"
                  alt="Enterprise custom software development team collaborating on a project"
                  loading="eager"
                  width={1200}
                  height={800}
        ...
        <section className="py-20" aria-labelledby="what-is-heading">
          <div className="mx-auto max-w-3xl px-6">
            <h2 id="what-is-heading" className="text-3xl font-extrabold text-[#0b1437] md:text-4xl">What Is Custom Software Development?</h2>
            {whatIsCustomSoftware.intro.map((p) => (
              <p key={p.slice(0, 40)} className="mt-5 text-[15px] leading-relaxed text-slate-700">{p}</p>
            ))}
            <h3 className="mt-10 text-xl font-bold text-[#0b1437]">Benefits of Custom Software Development</h3>
            <ul className="mt-4 space-y-3">
              {whatIsCustomSoftware.benefits.map((b) => (
            ...
            <p className="mt-4 text-[15px] leading-relaxed text-slate-700">{whatIsCustomSoftware.businessValue}</p>
            <h3 className="mt-10 text-xl font-bold text-[#0b1437]">Who Needs Custom Software?</h3>
            <ul className="mt-4 space-y-2">
              {whatIsCustomSoftware.whoNeeds.map((item) => (
        ...
        <section className="bg-slate-50 py-20" aria-labelledby="comparison-heading">
          <div className="mx-auto max-w-5xl px-6">
            <h2 id="comparison-heading" className="text-3xl font-extrabold text-[#0b1437] md:text-4xl">Why Businesses Choose Custom Software Over Off-the-Shelf Solutions</h2>
            {comparisonIntro.map((p) => (
              <p key={p.slice(0, 40)} className="mt-5 text-[15px] leading-relaxed text-slate-700">{p}</p>
            ))}
            <h3 className="mt-10 text-xl font-bold text-[#0b1437]">Custom Software vs. Off-the-Shelf Software</h3>
            <div className="mt-8 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-lg">
              <div className="grid grid-cols-3 bg-[#0f1a4e] text-sm font-bold text-white">
                ...
                <div className="border-l border-white/10 p-4 text-center">Custom Software</div>
        ...
        <section className="py-20" aria-labelledby="problems-heading">
          <div className="mx-auto max-w-5xl px-6">
            ...
            <p className="mt-4 max-w-3xl text-[15px] leading-relaxed text-slate-700">{problemsWeSolve.intro}</p>
            <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2">
              {problemsWeSolve.items.map((item, i) => (
            ...
            <p className="mt-8 text-[15px] leading-relaxed text-slate-700">{problemsWeSolve.closing}</p>
          </div>
        </section>
        ...
        <section className="bg-slate-50 py-20" aria-labelledby="services-heading">
          <div className="mx-auto max-w-5xl px-6">
            <h2 id="services-heading" className="text-3xl font-extrabold text-[#0b1437] md:text-4xl">Our Custom Software Development Services</h2>
            <div className="mt-10 space-y-6">
              {customSoftwareServices.map((s, i) => (
        ...
        <section className="py-20" aria-labelledby="types-heading">
          <div className="mx-auto max-w-5xl px-6">
            ...
            <p className="mt-4 text-[15px] text-slate-600">Our teams have experience building custom systems across a range of business functions and industry-specific applications, including:</p>
        ...
        <section id="process" className="bg-slate-50 py-20" aria-labelledby="process-heading">
          <div className="mx-auto max-w-7xl px-6">
            <h2 id="process-heading" className="text-center text-3xl font-extrabold text-[#0b1437] md:text-4xl">Our Custom Software Development Process</h2>
            <div className="mt-14 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {customSoftwareProcess.map(({ icon: Icon, title, desc }, i) => (
        ...
        <section className="py-20" aria-labelledby="tech-heading">
          <div className="mx-auto max-w-5xl px-6">
            ...
            <p className="mt-6 text-sm text-slate-600">We also work with microservices architecture where system complexity and independent scalability requirements justify it, alongside API integration work connecting custom software to third-party platforms and internal systems.</p>
          </div>
        </section>
            ...
            <article className="mt-8 rounded-2xl border border-dashed border-slate-300 bg-white p-8">
              <div className="space-y-6 text-sm">
                ...
                <div>
                  ...
                  <p className="mt-2 italic text-slate-600">Describe the custom software solution built, including key architecture or technology decisions.</p>
                </div>
                <div>
        ...
        <section className="py-20" aria-labelledby="portfolio-heading">
          <div className="mx-auto max-w-5xl px-6">
            ...
            <p className="mt-4 text-[15px] text-slate-600">Explore a selection of the custom software systems we&apos;ve built for clients across industries.</p>
